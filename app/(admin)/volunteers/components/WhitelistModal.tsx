...
export const WhitelistModal = ({ reloadTable }: { reloadTable: () => Promise<any> }) => {
  ...
  const handleSearch = async () => {
    ...
    const expireAt = form.getFieldValue('expireAt');
    await HttpClient.batchAddonVolunteerWhitelist({ ids, expireAt });
  ...
  return (
    ...
    >
      <ProFormGroup> 
        <ProFormDigit label="白名单时长" name="expireAt" width="xs" placeholder="单位：月" min={1} rules={[{ required: true }]} />
      </ProFormGroup>
